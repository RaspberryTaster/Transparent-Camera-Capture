...
public class zzTransparencyCapture
{
    ...
    /// <returns></returns>
    public static void ProcessScreenShots(string pFileName, string folderPath)
    {
        Texture2D screenShotResult = CaptureScreenshot();
        byte[] byteArray = screenShotResult.EncodeToPNG();

        File.WriteAllBytes(folderPath + $"/{pFileName}.png", byteArray);
    }
